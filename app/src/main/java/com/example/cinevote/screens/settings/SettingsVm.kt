...
class SettingsVm(private val repository: FilmRepository) : ViewModel() {
    ...
    val action = object : SettingsAction {
        ...
        override fun updateProfileImage(uri: Uri) {
            ...
            val profileData = hashMapOf(
                "user" to (firebaseAuth.currentUser?.email
                    ?: ""), // Sostituisci con l'ID dell'utente
            ...
            db.collection("profilePIC").add(profileData).addOnSuccessListener { documentReference ->
        ...
        override fun getFilmReviewd() {
            ...
            viewModelScope.launch(Dispatchers.IO) {
                val list = firestore.actions.getFilmReviewd()

                for (movie in list) {
                    val film = repository.getFilmFromTitle(movie)
                    val watched = WatchedMovie(
                        posterUrl = "${tmdbBaseUrl}${film.posterPath}", title = film.title
                    )
